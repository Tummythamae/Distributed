...
@WebServlet(name = "Swaziland", urlPatterns = {"/Swaziland"})
public class Swaziland extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            htmlTable.append("<h2>Swaziland Students</h2>");
            ...
            if (!foundResults) {
                // Display alert if no results were found
                htmlTable.append("<script>alert('No results found for the search query.'); window.location.href = 'Swaziland';</script>");
            }
            ...
            try (MongoCursor<Document> cursor = result.iterator()) {
                while (cursor.hasNext()) {
                    ...
                    htmlTable.append("<form action=\"TransferToLesotho\" method=\"get\">");
            ...
            // Error handling
            response.getWriter().println("An error occurred while retrieving Swaziland students: " + e.getMessage());
        }
    }
}
